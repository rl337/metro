...
class CitySimulator:
    ...
    def simulate_city(
        self, target_population: int = None, city_size: float = None
    ) -> CityLayout:
        """
    ...
    def _generate_districts(
        self, district_count: int, city_size: float, population: int
    ) -> List[District]:
        ...
        for i in range(district_count):
            ...
            district_seed = self.seed_manager.get_district_seed(
    ...
    def _select_weighted_choice(
        self, choices: Dict[str, float], rng: random.Random
    ) -> str:
    ...
    def _calculate_district_population(
        self,
        district_index: int,
        total_districts: int,
    ...
    def _generate_zones(
        self, districts: List[District], total_population: int
    ) -> List[Zone]:
        """Generate zones within districts."""
        zones = []
        ...
        for district in districts:
            ...
            district_rng = self.seed_manager.generator.get_rng(
                f"districts.{district.name.lower().replace(' ', '_')}"
            )
            ...
            for i in range(zone_count):
                ...
                zone_seed = self.seed_manager.get_zone_seed(
                    zone_type, district.name.lower().replace(" ", "_")
                )
    ...
    def _generate_road_network(
        self, districts: List[District], city_size: float, rng: random.Random
    ) -> List[Dict]:
        """Generate road network."""
        roads = []
    ...
    def _generate_utilities(
        self, districts: List[District], rng: random.Random
    ) -> Dict[str, List]:
    ...
    def _generate_demographics(
        self, districts: List[District], population: int
    ) -> Dict[str, Any]:
    ...
    def export_city_data(self) -> Dict[str, Any]:
        ...
        return {
            "metadata": {
                "generated_at": self.population_model.__dict__.get(
...
def simulate_city_from_config(
    config_path: str, target_population: int = None
) -> Dict[str, Any]:
    """
