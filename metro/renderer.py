...
class Renderer:
    ...
    def _render_thing(
        self, thing, scale: float, offset_x: float, offset_y: float
    ) -> None:
    ...
    def _render_rectangle(
        ...
        rect_patch = patches.Rectangle(
            (x, y), w, h, facecolor=color[:3], alpha=color[3]
        )
    ...
    def _render_group(
        ...
        for shape in group.get_shapes():
            ...
            temp_thing = SimpleThing(
                shape,
                Color.from_rgb(*[int(c * 255) for c in color[:3]], int(color[3] * 255)),
                0,
            )
